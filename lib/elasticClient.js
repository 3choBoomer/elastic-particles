...
function pageScroll(scrollId, rawData){
  ...
  function profileScroll(){
    log.profile('scroll ...' + scrollId.substr(scrollId.length-5, 5));
  }
}
...
function httpPost(path, body) {
  log.debug('Executing elastic query route: [%s] body: %s', path, body);
  return http.request({
    host: config.elastic.server,
    port: config.elastic.port,
    path: path,
...
function parseElasticResponse(response) {
  ...
  .then(function(body) {
    ...
    log.info('Elastic query returned in ' + resp.took + ' ms');
    return resp;
  });
}
